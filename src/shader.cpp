...
ShaderVariableType shadercross_to_shadervariable(const ::SDL_ShaderCross_IOVarType var_type) {
    ...
    throw Exception("unknown shader variable type: {}", std::to_underlying(var_type));
}
...
Shader::Shader(const std::string &source, ShaderType type)
    ...
    ::SDL_ShaderCross_HLSL_Info shader_info{source.c_str(), entrypoint, nullptr,
                                            nullptr,        stage,      0};
    _impl->shader_data = {
        static_cast<uint8_t *>(::SDL_ShaderCross_CompileSPIRVFromHLSL(&shader_info, &_size)),
        ::SDL_free};
    ...
    ensure(_impl->reflect, "unable to get shader reflection {}", SDL_GetError());

    _impl->info = {
        _impl->shader_data.get(),
        _size,
        entrypoint,
        stage,
        0,
    ...
    if (_type != ShaderType::VERTEX) return;

    for (int i = 0; i < _impl->reflect.get()->num_inputs; ++i) {
        auto [name, location, vector_type, vector_size] = _impl->reflect.get()->inputs[i];
        std::string clean {name};
        if (clean.starts_with("in.var.")) {
            clean.erase(0, 7);
        }

        _impl->variables[clean] = {
            clean,
...
const ::SDL_ShaderCross_GraphicsShaderResourceInfo *Shader::resource_info() const {
    return &_impl->reflect.get()->resource_info;
}

const std::unordered_map<std::string, ShaderVariableEntry> &Shader::variables() const { return _impl->variables; }
