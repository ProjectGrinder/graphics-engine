...
public:
    ...
    void draw(const Mesh &mesh, Material &material, Transform transform = {});
    void draw_end();

    template <typename Func>
    requires std::invocable<Func>
    void frame(Func &&draw_commands) {
        if (!draw_begin()) return;
        std::forward<Func>(draw_commands)();
        draw_end();
    }
    template <typename Func>
    requires std::invocable<Func>
    void frame(Func &&draw_commands, const float r, const float g, const float b, const float a) {
        if (!draw_begin(r, g, b, a)) return;
        std::forward<Func>(draw_commands)();
        draw_end();
    }
...
private:
    ...
    struct MaterialKeyHash {
        std::size_t operator()(const MaterialKey &key) const {
            return std::hash<void *>{}(key.vs) ^ (std::hash<void *>{}(key.ps) << 1);
        }
    };

    std::unordered_map<MaterialKey, ::SDL_GPUGraphicsPipeline *, MaterialKeyHash> _pipeline_cache;
};
